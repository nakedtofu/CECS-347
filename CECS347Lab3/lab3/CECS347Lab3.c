...
// Daniel Valvano, Peter Dome, Naoaki Takatsu
// February 22, 2018
...
void Delay(void){unsigned long volatile time;
  time = 727240*200/91;  // 0.1sec   200 => 400 
  while(time){
        time--;
  }
}
...
{ 
  ...
  if(GPIO_PORTF_RIS_R&0x10) // SW1 touch
    ...
    Delay();
		...
		{
			new_duty = period * .25;
			Nokia5110_SetCursor(7, 3);          // seven leading spaces, bottom row
	...
	if(speed_menu == 0)
	{
		...
		Nokia5110_SetCursor(3, 3);          // three leading spaces, bottom row
		Nokia5110_OutChar('S');
	}
...
int main(void)
{
  ...
  EnableInterrupts();   // enable after all initialization are done
	...
	Nokia5110_Init();
  Nokia5110_Clear();
  Nokia5110_OutString("* PWM Test *Dir: Speed%:---- -------");
